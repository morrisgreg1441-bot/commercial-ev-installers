---
to: membership@tile.org.uk
to_name: 
subject: £750–£1,500 directory build for The Tile Association members
slug: tile-association
prospect_rank: 40
status: ready
---
...
I build sector directories for trade bodies on fixed price. For The Tile Association, the worked example: a browsable directory of tile fixers, distributors and manufacturers by region × member type × specialism (wall, floor, swimming pool, large format), on the same site shell I built for the OZEV register.
...
Why relevant: a botched tiling job is expensive to put right, so homeowners and specifiers want a vetted fixer — a filterable TTA directory turns your membership into the obvious first search result for local tiling work.
